...
export const ColorSemanticTextAiDraft = "#ff1a8c"; // AI-drafted prose, not yet revised by human — Signalnoise hot pink. Brand-locked hex: same value across all themes so the AI marker is unmissable regardless of palette.
...
export const TypographyFontFamilyPrimary = ["Flexi IBM VGA True", "monospace"]; // Flexi IBM VGA True v2 (CC BY-SA 4.0) with a `monospace` fallback that preserves DOS aesthetic on @font-face failure.
export const TypographyFontFamilyFallback = ["monospace"]; // Bare `monospace` stack — fail-loud diagnostic counterpart to `primary`. Compose with `primary` when consumers want strict Flexi-or-nothing rendering.
