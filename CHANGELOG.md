...
### Fixed
- **Destructive Button is now honestly red (DMNC-1059).** The destructive variant painted raw `--color-cga-red`/`--color-cga-white`/`--color-cga-bright-red`, which in the amber-mono base palette rendered **brown-on-gold** (and failed the axe contrast gate). It now routes through the honest semantic roles — `background.error`/`border.error` (`cga-true.red` `#AA0000`) with `text.white` (`#FFFFFF`, ~5.9:1, passes AA). Hover/active emphasis comes from the red phosphor glow (`--color-cga-red-glow`) on a solid fill. Removed from `a11y-known-failures.json`. First component of the token-discipline sweep.

### Changed
- **Migrated all 92 `var(--color-cga-amber)` CSS usages to semantic brand roles across 31 components (DMNC-1059).** Each routed by property context: `color:` → `text.brand`, `background`/`-bg` → `background.brand`, `border`/`outline` → `border.brand` (incl. nested `var()` fallbacks, `color-mix` tints, and Terminal's local `--terminal-*` vars). All resolve to `#FFB000` today, so it's a visual no-op (Chromatic-verified) — but components now re-theme. Components are now off raw `--color-cga-amber` entirely (only `Card`'s `bg-cga-amber-dim` header remains, pending a subtle-surface role).
- **Migrated `text-cga-*` utilities to semantic roles across 9 components (DMNC-1059).** Stat, Input, Footer, Breadcrumb, Card, ChatInput, Accordion (Section + Fill), TimelineContainer. `text-cga-amber` → `text-dos-text-brand` (visual no-op, now themeable); Stat trend + Input error → honest functional roles: up → `text-success` (`#00AA00` green), down/error → `text-error` (`#FF5555` bright red, readable on dark — the gold `cga-bright-*` is gone). (`Card`'s `bg-cga-amber-dim` header is left pending a surface role.)

### Added
- New theme-invariant primitives `--color-cga-true-white` (`#FFFFFF`) + `--color-cga-true-red-bright` (`#FF5555`) and semantic roles `text.white` (light foreground for fills) — and `text.error` re-pointed to the bright red so error text passes AA on the dark CRT background (solid `#AA0000` stays for `background.error`/`border.error` fills).
