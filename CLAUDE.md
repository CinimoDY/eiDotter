...
**Tailwind CSS build (v0.16.1):** Tailwind v3 processes component utility classes via `postcss.config.cjs` + `tailwind.config.cjs`. The compiled `dist/eidotter.css` includes all Tailwind utilities — consumers do NOT need Tailwind installed to use components. `tailwind.preset` (all tokens + React Aria state variants + animate plugin) is the canonical preset export. `tailwind.preset.enhanced` (the deprecated alias from #282's split, restored as a shim in #283) was removed in v0.25.0 after its deprecation cycle completed — `tailwind.preset` is the only preset export.
