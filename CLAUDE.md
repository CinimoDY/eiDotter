...
## Current Component Status (v0.39.0, July 2026)

**Components** (52): Accordion, AIText, Alert, Badge, Brand (Logo, Wordmark, BrandLockup, Mark), Breadcrumb, Button, Card, ChatMessage, ChatHistory, ChatInput, ChatContainer, Checkbox, ChecklistRow, CmdPalette, CommandPrompt, Connector, CopyButton, DosFigure, EmptyState, FilterBar, Footer, Header, Icon, InlineExpand, InlineLink, Input, LabeledProgress, LedgerRow, LegalPage, Lightbox, Modal, Nav, Notification, Progress, RetroEffects, SectionHeading, Select, Separator, Skeleton, Stat, Switch, Tabs, Tag, Terminal, TextScramble, TimelineContainer, TimelineFeed, TimelineNode, TimelinePage, Tokens

**Connector system — nav-as-course (v0.39.0, DMNC-1388 / 1326 / 1325):** Four new components + a Header prop, shipped together:
- **`Header` `context` prop (DMNC-1326):** additive optional second row — a measurable category badge row (stable `.eidotter-header__context` / `.eidotter-header__categories` / `.eidotter-header__category` + `li[data-category-key]` measurement contract) + an optional "← back" returnTo pill (same-tab via `linkComponent`, or `reuseTab` named-tab reuse). Absent/empty `context` ⇒ DOM identical to before. Interactivity lives in an internal `'use client'` `HeaderContextRow` leaf so `Header` stays server-safe. New exported `isIconName()` guard. Category + returnTo hrefs guarded via the shared `isSafeHref`.
- **`Mark` (DMNC-1325, in `Brand/`):** a morphing soft-radial egg-yolk avatar with an optional `arms` mode drawing runtime-measured (`getBoundingClientRect`) quadratic-bezier SVG connectors to nav badges (default selector `.eidotter-header__category`). Mark-local coords (portal-free, scroll-invariant); `aria-hidden` arms; redraw on resize + badge-set change. `computeArmPaths` exported. Uses `useEffect` (not `useLayoutEffect`) because the scope is an ancestor whose ref attaches after a child's layout effect.
- **`Connector` (DMNC-1388):** organic tapered-ribbon SVG overlay — filled variable-width `<path>`s from a source node to N anchors, amber→category gradient (solid amber otherwise), stop-at-edge attach geometry, ResizeObserver + MutationObserver re-flow + compositor-friendly rAF idle sway (reduced-motion safe). Decorative (`aria-hidden`); pure geometry in `connectorGeometry.ts`.
- **`TimelineFeed` + `TimelinePage` (DMNC-1388):** two presentational surfaces sharing TimelineContainer's `TimelineEntryData` model. `TimelineFeed` = dense category-dot · date · title · kind-tag rows on a thin amber spine with a culling category filter. `TimelinePage` = Bret-Victor blog timeline — year-grouped, amber-only spine + dots, big titles + description, optional Mark brand slot + category filter, right-hand hover-preview panel.

(Note: this list also had pre-existing drift — CalendarHeatmap, HoldToComplete, ProgressRing, Provenance exist in `src/components/` but predate this entry; a full recount is a separate audit.)

**TimelineContainer `article` entry kind (v0.38.0, DMNC-1281):** A fourth `TimelineEntryData` kind (`kind: 'article'`) for devlog-style entries — additive union member, no new top-level component. Collapsed = a decorative thumbnail strip (≤4 thumbs + a `+N` overflow cell) + an 80-char `summary` preview, all inside the single expand/collapse trigger. Expanded = an interactive gallery (→ Lightbox) + a rich `content` body + a sanitized `href` "READ MORE →" link (`hrefLabel` overrides the label; unsafe schemes like `javascript:` render no anchor). The read-more/gallery live in the always-in-DOM `inert` collapsed body, so they're unreachable until expanded. Works in all four modes (interactive/feed/static/master-detail) with no per-mode change. The gallery `grid→focused→lightbox` state machine + its Lightbox mount were extracted into a shared internal `GalleryGrid` (`{ images, isExpanded }`) reused by both the `gallery` and `article` variants — no public API change, existing gallery tests unchanged. Consumer driver: dmnc.tech `/log`.
...
**RetroEffects `boot` (v0.28.0, DMNC-1047):** CGA monitor turn-on played once on mount (~650ms: ignition line → raster opens → warm phosphor wash). The portfolio-wide first-load signature — consumers add the one prop. Compositor-only; skipped under reduced motion; `onBootComplete` for sequencing boot text. Add `bootOnce` to gate it to once per browser tab/session (sessionStorage, key `bootStorageKey`, default `'eidotter:retro-boot'`): plays on first load, suppressed on later mounts in the same tab; a new tab/visit replays. Falls back to playing if storage is blocked.

**No-FOUC contract (DMNC-1184):** the boot sequence opens with a **full-viewport opaque blackout that ships in the server HTML** (`booting` starts from `boot` for both the gated and non-gated paths — the gated skip/replay decision is made post-mount), so the page is never visible before the turn-on — the animation IS the reveal. The two boot panels are full-height (not the old `height: 50%`, which left the center band exposed) and overlap into a complete cover at rest, then collapse to their own edges so the raster opens from the center outward. The open is **pure CSS**: it works before hydration and with JS disabled (the cover self-resolves to `scaleY(0)` — nothing stays permanently hidden), and `prefers-reduced-motion` keeps the cover `display: none` so content shows immediately. Regression-guarded by `renderToString` tests asserting the cover is in the server HTML. **`bootOnce` SSR trade:** because the cover ships server-side and the server can't read `sessionStorage`, a *returning* visitor's full-reload/hard-refresh briefly shows the blackout before JS suppresses it post-hydration (a brief blackout on a fast host — the 140ms open delay means the turn-on usually isn't reached before teardown — never a content flash). SPA route changes keep the overlay mounted in the layout and never show it. Eliminating that brief returning-visitor blackout entirely needs server-visible session state (cookie / pre-paint inline script), which is consumer-side.
...
**Two-tier typography (v0.37.0, DMNC-885):** Display tier (Perfect DOS VGA 437, `--typography-font-family-primary`, classes `.dos-page`/`.dos-hero`/`.dos-h1–h5`/`.dos-label`/`.dos-code`) vs body tier (JetBrains Mono Nerd Font, `--typography-font-family-body`, classes `.dos-body`/`.dos-body-lg`/`.dos-caption`/`.dos-micro`). Body tier supports real weights 400/500/600/700 via `--typography-font-weight-body-{regular,medium,semibold,bold}`. Nerd Font glyphs (powerline, devicons) available in body text. 4 static woff2 files shipped in `src/styles/fonts/` (no variable woff2 — Nerd Fonts v3.4.0 only releases static TTFs; converted via fonttools+brotli). OFL-1.1 license included as `JetBrainsMonoNerdFont_OFL.txt`.
...
- **CI guardrails (post-v0.20.0):** `main` is branch-protected; `build (22.x)` must be green before merge (strict mode — CI re-runs on every HEAD). `build.yml` runs: token freshness check → `npm run lint-tokens` → build → `npm test` (1244 tests) → build storybook. The token-ref lint (`scripts/lint-token-refs.mjs`) catches any `var(--*)` ref or `text-dos-*`/`bg-dos-*`/etc. Tailwind class that doesn't resolve to a declared token or preset key. The preset parity contract (`src/styles/tailwind-preset.test.ts`) asserts every `semanticVarMap` entry in `style-dictionary.config.mjs` has a matching `theme.extend.colors` key. Never add `--no-verify`, `--admin`, or force-push workarounds to bypass — fix the underlying drift.
...
- **RSC / server-component imports (`eidotter/components/<Name>`, DMNC-1130):** The `.` barrel (`dist/index.es.js`) inlines all components with **zero** `'use client'` directives, so importing it into a Next.js **server component** crashes RSC (the DMNC-864 constraint). For server components, deep-import per component: `import { SectionHeading } from 'eidotter/components/SectionHeading'` (PascalCase, matches the source dir). The `./components/*` wildcard export points at an additive per-component ESM emit (`dist/components/<Name>/index.js`, built by `tsconfig.components.json` + `scripts/finalize-component-emit.mjs`) where interactive components carry `'use client'` and presentational ones don't — so presentational primitives render on the server and interactive ones resolve as client references. CSS still ships once via `eidotter/styles`. The `.` barrel is unchanged — still the default for client apps. **Adding a component automatically joins this surface** (the build pass + `package-exports.test.ts` parity cover every component dir). The directive↔interactivity invariant is **source-of-truth** via `scripts/sync-client-directives.mjs` (classifier in `scripts/lib/client-components.mjs`; `forwardRef` alone is NOT a client signal) and CI-guarded by `--check` + `scripts/assert-rsc-safety.mjs`. Import-only (no `require`) — ESM/bundler consumers only, like `./icons/*`.
