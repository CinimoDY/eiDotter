...
- **Icons:** Backed by [`pixelarticons`](https://github.com/halfmage/pixelarticons) (MIT) — authentic DOS pixel art matching eidotter's aesthetic. Wrapped in `src/components/Icon/components/Icon.tsx` (ICON_MAP with 12 mapped names).
...
**Icons (v0.18.0+):** Backed by [`pixelarticons`](https://github.com/halfmage/pixelarticons) (MIT licensed, ~480 icons, authentic DOS pixel art style). The `<Icon>` component wraps 11 pixelarticons via an internal ICON_MAP — consumers use `<Icon name="Warning" size="S" />` unchanged. Available names: Info, Warning, Error, Done, Check, Close, Chevron Up, Chevron Down, App, Cancel, Fullscreen, Add. Earlier versions used `@untitledui-pro/icons` — that was removed for license reasons (UTI Pro is not sublicensable; bundling it into dist was a license violation). The old SVG spritesheet was removed in v0.18.0.
